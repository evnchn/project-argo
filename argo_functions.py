...
def makelink(a, bfp, endpoint):
    return f"<a href='{endpoint+bfp}#page={a}'>'{a}'</a>"
...
def search_pptx_files(word, output_stream, http_mode=False, endpoint=""):
    ...
    for file_path in pptx_files:
        ...
        if http_mode:
            ...
            if found_pages:
                output_stream.write(f"Word '{word}' found in <a href='{endpoint+bfp2}'>'{bfp2}'</a>, on page(s): {','.join(makelink(a, bfp2, endpoint) for a in map(str, found_pages))}<br>")
            else:
                output_stream.write(f"Word '{word}' not found in <a href='{endpoint+bfp2}'>'{bfp2}'</a>.<br>")
        else:
            if found_pages:
